...
<div class="page-heading">
    <div class="page-heading-copy">
        ...
        <div>
            ...
            <p class="text-muted mb-0">Update service: <strong>{{ $service->name }}</strong></p>
        </div>
    </div>
...
<section class="row g-3">
    <div class="col-12 col-xl-8">
        <form class="panel" action="{{ route('admin.service.update', $service->id) }}" method="POST" enctype="multipart/form-data" id="serviceForm">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input class="form-control @error('name') is-invalid @enderror"
                           name="name" type="text" value="{{ old('name', $service->name) }}" required>
                    @error('name')
                ...
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        <input class="form-control @error('slug') is-invalid @enderror"
                               id="slug" name="slug" type="text" value="{{ old('slug', $service->slug) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <input class="form-control @error('sort_order') is-invalid @enderror"
                           name="sort_order" type="number" value="{{ old('sort_order', $service->sort_order) }}" min="0">
                    @error('sort_order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    @if($service->thumbnail)
                        <div class="mb-2 p-2 bg-light rounded">
                            <img src="{{ $service->thumbnail_url }}" width="100" class="rounded" alt="{{ $service->name }}">
                        </div>
                    @endif
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror ckeditor"
                              id="serviceDescription" name="description" rows="15">{{ old('description', $service->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="panel h-100">
            ...
            <div class="activity-list">
                <div class="activity-item">
                    <span class="activity-dot {{ $service->name ? 'bg-success' : 'bg-secondary' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Name & Slug</p>
                        <p class="text-muted small">{{ $service->name ? 'Name is set' : 'Name is missing' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $service->thumbnail ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Thumbnail</p>
                        <p class="text-muted small">{{ $service->thumbnail ? 'Thumbnail is set' : 'Thumbnail is missing' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $service->description ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Description</p>
                        <p class="text-muted small">{{ $service->description ? 'Description is set' : 'Description is missing' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $service->status == 'active' ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Status</p>
                        <p class="text-muted small">Service is {{ $service->status }} on frontend</p>
                    </div>
                </div>
            </div>
        </div>
