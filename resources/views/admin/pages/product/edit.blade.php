...
<section class="row g-3">
    <div class="col-12 col-xl-8">
        <form class="panel" action="{{ route('admin.product.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        <input class="form-control @error('slug') is-invalid @enderror"
                               id="slug" name="slug" type="text" value="{{ old('slug', $product->slug) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <input class="form-control @error('sort_order') is-invalid @enderror"
                           name="sort_order" type="number" value="{{ old('sort_order', $product->sort_order) }}" min="0">
                    @error('sort_order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('new') is-invalid @enderror" name="new" required>
                        <option value="inactive" {{ old('new', $product->new) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                        <option value="active" {{ old('new', $product->new) == 'active' ? 'selected' : '' }}>Active</option>
                    </select>
                    @error('new')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('trending') is-invalid @enderror" name="trending" required>
                        <option value="inactive" {{ old('trending', $product->trending) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                        <option value="active" {{ old('trending', $product->trending) == 'active' ? 'selected' : '' }}>Active</option>
                    </select>
                    @error('trending')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('show_on_home_page') is-invalid @enderror" name="show_on_home_page" required>
                        <option value="inactive" {{ old('show_on_home_page', $product->show_on_home_page) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                        <option value="active" {{ old('show_on_home_page', $product->show_on_home_page) == 'active' ? 'selected' : '' }}>Active</option>
                    </select>
                ...
                <div class="col-12">
                    ...
                    @if($product->thumbnail)
                        <div class="mb-2 p-2 bg-light rounded">
                            <img src="{{ $product->thumbnail_url }}" width="100" class="rounded" alt="{{ $product->name }}">
                        </div>
                    @endif
                ...
                @if($product->galleries->count() > 0)
                    <div class="col-12">
                        <label class="form-label">Current Galleries</label>
                        <div class="d-flex flex-wrap gap-2">
                            @foreach($product->galleries as $gallery)
                                <div class="position-relative p-2 bg-light rounded">
                                    <img src="{{ $gallery->image_url }}" width="100" height="100" class="rounded object-fit-cover" alt="Gallery">
                                    <div class="form-check mt-1">
                                        <input class="form-check-input" type="checkbox"
                                               name="remove_galleries[]" value="{{ $gallery->id }}"
                                               id="removeGallery{{ $gallery->id }}">
                                        <label class="form-check-label text-danger small" for="removeGallery{{ $gallery->id }}">
                ...
                <div class="col-12">
                    <label class="form-label">Add New Galleries</label>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('short_description') is-invalid @enderror"
                              ...
                              placeholder="Brief summary of the product">{{ old('short_description', $product->short_description) }}</textarea>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror ckeditor"
                              id="productDescription" name="description" rows="15">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="panel h-100">
            ...
            <div class="activity-list">
                <div class="activity-item">
                    <span class="activity-dot {{ $product->name ? 'bg-success' : 'bg-secondary' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Name & Slug</p>
                        <p class="text-muted small">{{ $product->name ? 'Name is set' : 'Name is missing' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $product->brand_id ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Brand</p>
                        <p class="text-muted small">{{ $product->brand_id ? 'Brand assigned' : 'No brand assigned' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $product->category_id ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Category</p>
                        <p class="text-muted small">{{ $product->category_id ? 'Category assigned' : 'No category assigned' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $product->thumbnail ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Thumbnail</p>
                        <p class="text-muted small">{{ $product->thumbnail ? 'Thumbnail is set' : 'Thumbnail is missing' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $product->description ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Description</p>
                        <p class="text-muted small">{{ $product->description ? 'Description is set' : 'Description is missing' }}</p>
                    </div>
                </div>
                <div class="activity-item">
                    <span class="activity-dot {{ $product->status == 'active' ? 'bg-success' : 'bg-warning' }}"></span>
                    <div>
                        <p class="mb-1 fw-semibold">Status</p>
                        <p class="text-muted small">Product is {{ $product->status }} on frontend</p>
                    </div>
                </div>
            </div>
        </div>
